...
# Plot scatter points for each doctor
...
ax.text(len(patients_df) * 0.98, dreamy_mean, 
        f'○ {dreamy_mean:.2f}', 
        ha='right', va='bottom', fontsize=11, 
        color='#4E79A7', weight='bold', 
        bbox=dict(boxstyle='round,pad=0.3', facecolor='white', 
                  edgecolor='#4E79A7', alpha=0.8), zorder=4)
ax.text(len(patients_df) * 0.98, duck_mean, 
        f'△ {duck_mean:.2f}', 
        ha='right', va='top', fontsize=11, 
        color='#E15759', weight='bold', 
        bbox=dict(boxstyle='round,pad=0.3', facecolor='white', 
                  edgecolor='#E15759', alpha=0.8), zorder=4)

# Styling
ax.set_xlabel('Patient Number', fontsize=12, weight='bold')
...
ax.set_xlim(0, len(patients_df) + 1)
...
# Add horizontal dashed lines for means
ax.axhline(y=dreamy_mean_randomized, color='#4E79A7', linestyle='--', linewidth=2, 
           alpha=0.8, zorder=2)
ax.axhline(y=duck_mean_randomized, color='#E15759', linestyle='--', linewidth=2, 
           alpha=0.8, zorder=2)

# Add text annotations with mean values and shape symbols
ax.text(len(patients_randomized_df) * 0.98, dreamy_mean_randomized, 
        f'○ {dreamy_mean_randomized:.2f}', 
        ha='right', va='bottom', fontsize=11, 
        color='#4E79A7', weight='bold', 
        bbox=dict(boxstyle='round,pad=0.3', facecolor='white', 
                  edgecolor='#4E79A7', alpha=0.8), zorder=4)
ax.text(len(patients_randomized_df) * 0.98, duck_mean_randomized, 
        f'△ {duck_mean_randomized:.2f}', 
        ha='right', va='top', fontsize=11, 
        color='#E15759', weight='bold', 
        bbox=dict(boxstyle='round,pad=0.3', facecolor='white', 
                  edgecolor='#E15759', alpha=0.8), zorder=4)
...
ax.legend(loc='upper right', frameon=True, fancybox=True, 
          shadow=True, fontsize=11)
ax.set_xlim(0, len(patients_randomized_df) + 1)
...
# Separate data by doctor (using observational data already loaded)
dreamy_data = patients_df[patients_df['doctor_name'] == 'Doc Dreamy']
duck_data = patients_df[patients_df['doctor_name'] == 'Doc Duck']

# Filter data to only include patients with severity between -1 and 1
dreamy_data_filtered = dreamy_data[(dreamy_data['severity'] >= -1) & (dreamy_data['severity'] <= 1)]
duck_data_filtered = duck_data[(duck_data['severity'] >= -1) & (duck_data['severity'] <= 1)]

# Calculate means for patients within severity range -1 to 1 only
dreamy_mean = dreamy_data_filtered['post_surgical_score'].mean()
duck_mean = duck_data_filtered['post_surgical_score'].mean()

# Create figure with professional styling
fig, ax = plt.subplots(figsize=(8, 6))

# Add shaded region highlighting severity between -1 and 1
ax.axvspan(-1, 1, alpha=0.15, color='gray', zorder=0, label='Comparison Region')
...
ax.text(x_max - 0.05 * x_range, dreamy_mean, 
        f'○ {dreamy_mean:.2f}', 
        ha='right', va='bottom', fontsize=11, 
        color='#4E79A7', weight='bold', 
        bbox=dict(boxstyle='round,pad=0.3', facecolor='white', 
                  edgecolor='#4E79A7', alpha=0.8), zorder=4)
ax.text(x_max - 0.05 * x_range, duck_mean, 
        f'△ {duck_mean:.2f}', 
        ha='right', va='top', fontsize=11, 
        color='#E15759', weight='bold', 
        bbox=dict(boxstyle='round,pad=0.3', facecolor='white', 
...
# Styling
ax.set_xlabel('Initial Patient Severity', fontsize=12, weight='bold')
ax.set_ylabel('Post-Surgical Symptom Score (Lower is Better)', 
              fontsize=12, weight='bold')
...
### Challenge 1: Observational Data Analysis

The first challenge demonstrated how observational data can be misleading. Figure 1 revealed that Doc Dreamy appeared to have superior outcomes (mean score 2.80) compared to Doc Duck (mean score 3.38), with a statistically significant difference (p = 0.023). However, this conclusion proved to be a false impression created by confounding—specifically, patient severity influenced both surgeon selection and surgical outcomes. This challenge highlighted the critical danger of drawing causal conclusions from observational data without accounting for potential confounders.

### Challenge 2: Randomized Controlled Trial

The second challenge illustrated the power of randomization to break confounding relationships. Figure 2 showed the true causal effect when patients were randomly assigned to surgeons: Doc Duck performed significantly better (mean score 2.71) than Doc Dreamy (mean score 3.46), with a statistically significant difference (p = 0.007). By breaking the link between patient severity and surgeon choice, randomization revealed that Doc Duck is actually the superior surgeon—the opposite conclusion from the observational data. This challenge demonstrated that randomization is the gold standard for establishing causality by eliminating confounding.

### Challenge 3: Stratification Analysis

The third challenge demonstrated stratification as an alternative method when randomization is not possible. Figure 3 visualized how patient severity confounded the relationship between surgeon choice and outcomes. By examining patients within similar severity ranges (specifically between -1 and 1), the stratification approach revealed that Doc Duck actually outperformed Doc Dreamy within comparable patient groups. This challenge showed that when randomization is infeasible, stratifying by the confounder can help reveal the true relationship between treatment and outcome, allowing researchers to compare like-with-like rather than being misled by aggregate statistics.
